import { majorInterval, minorInterval } from "./scales"
...
function binarySearch(scale, note) {
  ...
  function findMatches(scale, notes) {
    let matches = [];

    for (let i = 0; i < notes.length; i++) {
      let index = binarySearch(scale, notes[i]);
      if (index === -1) {
        let closest = findClosest(scale, notes[i]);
        matches.push(scale[closest]);
      } else {
        matches.push(scale[index]);
      }
    }
    return matches;
  }
